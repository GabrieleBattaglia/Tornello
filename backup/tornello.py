...
def sincronizza_db_personale():
    ...
    for player_id, local_player in players_db.items():
        ...
        if fide_id_str and fide_id_str != "0":
            ...
            if not p_first_name or not p_last_name:
                continue
            ...
            if len(matches) == 1:
                ...
                print(
                    _(
                        "\n-> Trovata una corrispondenza FIDE per il tuo giocatore '{} {}' (ID: {}):"
                    ).format(
                        local_player.get("first_name"),
                        local_player.get("last_name"),
                        player_id,
                    )
                )
                print(
                    _(
                        "   FIDE ID: {}, Nome: {}, {}, FED: {}, Elo: {}, Anno Nascita: {}"
                    ).format(
                        match["id_fide"],
                        match["last_name"],
                        match["first_name"],
                ...
                if enter_escape(
                    _("   Associare questo ID FIDE al tuo giocatore? (INVIO|ESCAPE)")
                ):
                ...
                print(
                    _(
                        "\n-> Trovati {} omonimi nel DB FIDE per il tuo giocatore '{} {}' (ID: {})."
                    ).format(
                        len(matches),
                        local_player.get("first_name"),
                        local_player.get("last_name"),
                ...
                if choice.isdigit() and 1 <= int(choice) <= len(matches):
                    ...
                    print(
                        _(
                            "   Scelta non valida o saltata. Il giocatore non verrà associato."
                        )
                    )
    ...
    if not all_potential_changes:
        ...
        return
    print(
        _("\n--- Riepilogo Sincronizzazione: Trovate {} modifiche proposte ---").format(
            len(all_potential_changes)
        )
    )
    for change in all_potential_changes[:3]:
        player = change["current_data"]
        print(
            _(" - Giocatore: {} {} (ID Locale: {})").format(
                player.get("first_name"), player.get("last_name"), player.get("id")
            )
        )
        if change["new_fide_id"]:
            print(
                _("    -> Associazione nuovo ID FIDE: {}").format(change["new_fide_id"])
            )
        if change["updates"]:
            for key, value in change["updates"].items():
                print(
                    _("    -> Aggiornamento {}: da '{}' a '{}'").format(
                        key.replace("_", " ").title(), player.get(key), value
                    )
                )
    ...
    if len(all_potential_changes) > 3:
        if (
            enter_escape(
                _(
                    "\nVuoi vedere l'elenco completo di tutte le modifiche proposte? (INVIO|ESCAPE)"
                )
            )
            == "s"
        ):
            ...
            for change in all_potential_changes:  # Mostra tutti
                ...
                if change["new_fide_id"]:
                    print(
                        _(" -> Associazione nuovo ID FIDE: {}").format(
                            change["new_fide_id"]
                        )
                    )
                if change["updates"]:
...
def gestisci_pianificazione_partite(torneo, current_round_data, players_dict):
    ...
    def _input_schedule_details(existing_details=None):
        ...
        while True:
            ...
            if (
                not date_input_str and is_modifying
            ):  # Mantiene il vecchio valore se input vuoto in modifica
            ...
            try:
                if (
                    "-" in date_input_str or "/" in date_input_str
                ):  # Formato con separatori
                    ...
                    date_input_str.isdigit() and 1 <= len(date_input_str) <= 2
                ):  # Solo Giorno (GG)
                ...
                TypeError
            ):  # In caso parsed_date_obj rimanga None per un path non gestito
                print(
        ...
        while True:
            ...
            prompt_time += _(" [Attuale: {current}]: ").format(
                current=current_display_time
            )
    ...
    while True:
        all_matches_in_round_sorted_by_id = sorted(
            current_round_data.get("matches", []), key=lambda m: m.get("id", 0)
        )
        if not any(
            m.get("result") is None and m.get("black_player_id") is not None
            for m in all_matches_in_round_sorted_by_id
        ):
            print(
                _(
                    "\nTutte le partite del Turno {round_num} sono state giocate o sono BYE."
        ...
        actual_board_number_counter = (
            0  # Contatore per il "Numero Scacchiera del Turno"
        )
        ...
        for match_obj in all_matches_in_round_sorted_by_id:
            ...
            if match_obj.get("black_player_id") is None:
                continue
            actual_board_number_counter += (
                1  # Incrementa per ogni partita giocabile (non BYE)
            )
            ...
            wp_name = (
                f"{wp.get('first_name', '?')} {wp.get('last_name', '?')}"
                if wp
                else "N/A"
            )
            bp_name = (
                f"{bp.get('first_name', '?')} {bp.get('last_name', '?')}"
                if bp
                else "N/A"
            )
            ...
            if match_obj.get("is_scheduled", False) and match_obj.get("schedule_info"):
                ...
                display_lines_already_scheduled.append(
                    _(" Pianificata per: {date} alle {time}").format(
                        date=date_str, time=time_str
                    )
                )
                display_lines_already_scheduled.append(
                    _(" Canale: {channel}, Arbitro: {arbiter}").format(
                        channel=channel_str, arbiter=arbiter_str
                    )
                )
            else:
        ...
        print(
            _("\n--- Pianificazione Partite Turno {round_num} ---").format(
                round_num=current_round_num
            )
        )
        print(
            _(
                "(Vengono mostrati i Numeri Scacchiera reali del turno per le partite ancora da giocare)"
            )
        )

        print(_("\nPartite da Pianificare (non hanno ancora una pianificazione):"))
        if display_lines_pending_scheduling:
            for line in display_lines_pending_scheduling:
                print(line)
        else:
            print(
                _(
                    "  Nessuna partita attualmente da pianificare (o sono tutte già pianificate)."
                )
            )

        print(_("\nPartite Già Pianificate (in attesa di risultato):"))
        if display_lines_already_scheduled:
            for line in display_lines_already_scheduled:
                print(line)
        else:
        ...
        action = (
            key(
                _(
                    "\nOpzioni: (P)ianifica, (M)odifica/Rimuovi pianificazione, (ESCAPE) termina: "
                )
            )
            .strip()
            .lower()
        ...
        if action == "\x1b":
            ...
            if action == "p":
                prompt_msg += _(" da pianificare: ")
            else:
                prompt_msg += _(" la cui pianificazione vuoi modificare/rimuovere: ")
            ...
            if selected_board_id not in round_board_to_global_id_map:
                print(
                    _(
                        "N#. Scacchiera non valido o non corrisponde a una partita gestibile."
                    )
                )
                continue

            target_match_global_id = round_board_to_global_id_map[selected_board_id]
            is_currently_scheduled = round_board_is_scheduled_map.get(
                selected_board_id, False
            )  # Default a False
            ...
            if not match_object_to_update:
                print(
                    _("ERRORE INTERNO: Partita IDG {match_id} non trovata.").format(
                        match_id=target_match_global_id
                    )
                )
                continue
            ...
            if action == "p":
                if is_currently_scheduled:
                    print(
                        _("Questa partita è già pianificata. Usa 'M' per modificarla.")
                    )
                    continue
                print(
                    _(
                        "\nPianificazione per la partita Sc. {board_id} (IDG: {match_id})..."
                    ).format(
                        board_id=selected_board_id, match_id=target_match_global_id
                    )
                )
                ...
                if not is_currently_scheduled:
                    print(
                        _(
                            "Questa partita non ha una pianificazione da modificare/rimuovere. Usa 'P'."
                        )
                    )
                    continue
                ...
                wp_name_m = (
                    f"{wp_m.get('first_name', '?')} {wp_m.get('last_name', '?')}"
                    if wp_m
                    else "N/A"
                )
                bp_name_m = (
                    f"{bp_m.get('first_name', '?')} {bp_m.get('last_name', '?')}"
                    if bp_m
                    else "N/A"
                )
                print(
                    _(
                        "\nGestione pianificazione per Sc. {board_id} (IDG: {match_id}): {white_player} vs {black_player}"
                    ).format(
                        board_id=selected_board_id,
                        match_id=target_match_global_id,
                        white_player=wp_name_m,
                        black_player=bp_name_m,
                    )
                )
                ...
                sub_action = (
                    key(
                        _(
                            "Vuoi (M)odificare o (R)imuovere la pianificazione? (Invio per annullare): "
                        )
                    )
                    .strip()
                    .lower()
                ...
                if sub_action == "m":
                    updated_schedule_data = _input_schedule_details(
                        existing_details=match_object_to_update.get("schedule_info")
                    )
                    if updated_schedule_data:
                        if updated_schedule_data != match_object_to_update.get(
                            "schedule_info"
                        ):  # Controlla se c'è stato un cambiamento reale
                            match_object_to_update["schedule_info"] = (
                    ...
                    print(_("Azione non valida per modifica/rimozione."))
        else:
...
def crea_nuovo_giocatore_nel_db(
    ...
    print(
        ...
        ).format(first_name=norm_first, last_name=norm_last, player_id=new_player_id)
    )
...
def calculate_dates(start_date_str, end_date_str, total_rounds):
    ...
    try:
        ...
        if total_duration < total_rounds:
            print(
                _(
                    "Attenzione: La durata totale ({duration} giorni) è inferiore al numero di turni ({rounds})."
...
def generate_pairings_for_round(torneo):
    ...
    players_sorted_for_start_rank = sorted(
        ...
        key=lambda p: (
            -float(p.get("initial_elo", DEFAULT_ELO)),
...
def handle_bbpairings_failure(torneo, round_number, error_message):
    ...
    print(
        _(
            "Oppure, considerare di effettuare gli abbinamenti per questo turno manualmente (su carta)."
        )
    )
    ...
    while True:
        ...
        if not data_input:  # Logica per terminare l'inserimento
            if current_num_players < 2:
                if enter_escape(
                    _("Ci sono meno di 2 giocatori. Continuare? (INVIO|ESCAPE)")
                ):
                    break
                else:
                    continue
        ...
        if potential_id_input in players_db:
            ...
            if len(matches_in_personal_db) == 1:
                ...
                print(
                    _(
                        "Trovati {count} giocatori nel tuo DB personale. Specifica usando l'ID locale:"
                    ).format(count=len(matches_in_personal_db))
                )
                for i, p in enumerate(matches_in_personal_db):
                    print(
                        f"  {i + 1}. ID: {p.get('id')} - {p.get('first_name')} {p.get('last_name')}"
                    )
                continue

        if player_id_to_add:
            print(
                _("Trovato giocatore nel DB personale: {} (ID: {})").format(
                    players_db[player_id_to_add].get("first_name"), player_id_to_add
                )
            )
        ...
        if not player_id_to_add:
            print(
                _(
                    "Giocatore non trovato nel DB personale. Avvio ricerca nel DB FIDE..."
                )
            )
            ...
            if len(fide_matches) == 1:
                ...
                while True:  # Loop per la paginazione
                    ...
                    if has_more_pages:
                        prompt_text += _(
                            "Scelta (Numero, 0 per manuale, Invio per i prossimi {page_size}): "
        ...
        if player_id_to_add and player_data_from_db:
            if player_id_to_add in added_player_ids_to_tournament:
                ...
                print(
                    _(
                        "-> Giocatore '{first_name} {last_name}' (ID DB: {player_id}) aggiunto al torneo."
                    ).format(
...
def update_match_result(torneo):
    ...
    while True:
        ...
        if not user_input_str:
            ...
            if gestisci_pianificazione_partite(
                torneo, current_round_data, players_dict
            ):
                any_changes_made_in_this_session = True
            continue
        ...
        if selected_match_obj_for_processing:
            ...
            result_input = dgt(
                _("Risultati: [1-0, 0-1, 1/2, 0-0F, 1-F, F-1]: "),
                kind="s",
                smin=3,
                smax=4,
            )
    ...
    try:
        with open(filename, "w", encoding="utf-8-sig") as f:
            ...
            if not scheduled_pending_active:
                f.write(
                    _(
                        "  (Nessuna partita con giocatori attivi è attualmente pianificata con data/ora)\n"
                    )
                )
            else:
...
def save_standings_text(torneo, final=False):
    ...
    if final:
        ...
        if not has_any_results and current_round_in_state == 1:
            ...
            if (
                not final
                and current_round_in_state > 0
                and current_round_in_state <= torneo.get("total_rounds", 0)
            ):
    ...
    try:
        with open(filename, "w", encoding="utf-8-sig") as f:
            ...
            header_table = _(
                "Pos. Titolo Nome Cognome               [EloIni] Punti  Bucch-1  Bucch    ARO "
            )
            if final:
            ...
            for player in players_sorted:
                ...
                rank_display_str = (
                    f"{int(rank_to_show):>3}."
                    if isinstance(rank_to_show, (int, float))
                    else f"{str(rank_to_show):>3} "
                )
...
def display_status(torneo):
    ...
    if current_round_dates:
        ...
        try:
            ...
            round_end_dt = datetime.strptime(r_end_str, DATE_FORMAT_ISO).replace(
    ...
    try:
        tournament_end_dt = datetime.strptime(end_d_str, DATE_FORMAT_ISO).replace(
            hour=23, minute=59, second=59
        )
...
if __name__ == "__main__":
    ...
    if not potential_tournament_files:
        ...
        if (
            ...
            print(
                _(" {num}. Crea un nuovo torneo").format(
    ...
    if deve_creare_nuovo_torneo or torneo is None:
            ...
            while True:  # Loop solo per la richiesta del nome se necessario
                ...
                if input_corrente_nome_torneo:
                    ...
                    prospective_filename = (
                        f"Tornello - {sanitized_name_new}.json"  # Usa il tuo stile
                    )
        ...
        while True:
            try:
                ...
                print(
                    _("Formato data non valido. Usa {date_format}. Riprova.").format(
                        date_format=DATE_FORMAT_ISO
                    )
                )
        while True:
            try:
                ...
                print(f"Formato data non valido. Usa {DATE_FORMAT_ISO}. Riprova.")
            except OverflowError:
                print(_("Errore: La data calcolata risulta troppo lontana nel futuro."))
                continue
        while True:
            try:
        ...
        fed_code = (
            input(_("  Federazione organizzante (codice 3 lettere) [Default: ITA]: "))
            .strip()
            .upper()
            or "ITA"
        )
