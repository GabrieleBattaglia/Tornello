...
def display_player_details(player_data):
    ...
    print(f"{_('Nome Completo'):<30}: {title_prefix}{player_data.get('first_name', 'N/D')} {player_data.get('last_name', 'N/D')}")
    print(f"{_('Titolo FIDE'):<30}: {player_data.get('fide_title', 'N/D') if player_data.get('fide_title') else _('Nessuno')}")
    print(f"{_('Elo Corrente'):<30}: {player_data.get('current_elo', 'N/D')}")
    print(f"{_('Sesso'):<30}: {str(player_data.get('sex', 'N/D')).upper()}")
    print(f"{_('Federazione'):<30}: {str(player_data.get('federation', 'N/D')).upper()}")
    print(f"{_('ID FIDE Numerico'):<30}: {player_data.get('fide_id_num_str', 'N/D')}")
    print(f"{_('Data Nascita'):<30}: {format_date_locale(player_data.get('birth_date'))}") 
    print(f"{_('Data Registrazione DB'):<30}: {format_date_locale(player_data.get('registration_date'))}")
    print(f"{_('Partite Giocate (valutate)'):<30}: {player_data.get('games_played', 0)}")
    ...
    print(f"{_('Medagliere'):<30}: Oro: {medals.get('gold',0)}, Argento: {medals.get('silver',0)}, Bronzo: {medals.get('bronze',0)}, Legno: {medals.get('wood',0)}")
...
def add_new_player(players_db_main_dict):
    ...
    if not first_name: print(_("Nome richiesto.")); return False
    last_name = get_input_with_default(_("Cognome: ")).title()
    if not last_name: print(_("Cognome richiesto.")); return False
    
    elo_val = dgt(f"{_('Elo Corrente')} (default {int(DEFAULT_ELO)})", kind="i", imin=500, imax=4000, default=int(DEFAULT_ELO))
    fide_title_new = get_input_with_default(_("Titolo FIDE (es. FM, o vuoto per nessuno):"), "").upper()[:3]
    ...
    federation_new = get_input_with_default(_("Federazione (3 lettere):"), "ITA").upper()[:3] or "ITA"
    fide_id_num_new = get_input_with_default(_("ID FIDE Numerico (cifre, '0' se N/D):"), "0")
    if not fide_id_num_new.isdigit(): fide_id_num_new = '0'

    birth_date_str_new = None
    while True:
        bdate_input = get_input_with_default(_("Data di nascita ({date_format} o vuoto):").format(date_format=DATE_FORMAT_ISO), "")
        if not bdate_input: break
        try:
            datetime.strptime(bdate_input, DATE_FORMAT_ISO)
            birth_date_str_new = bdate_input
            break
        except ValueError: print(_("Formato data non valido. Usa {date_format}.").format(date_format=DATE_FORMAT_ISO))
    
    experienced_new_val = False
    while True:
        exp_input_str = get_input_with_default(_("Giocatore con esperienza pregressa significativa? (s/n):"), "n").lower()
    ...
    new_id = crea_nuovo_giocatore_nel_db(
        players_db_main_dict, first_name, last_name, float(elo_val),
        fide_title_new, sex_new, federation_new, fide_id_num_new, 
        birth_date_str_new, experienced_new_val, silent=False
    )
    if new_id:
...
def edit_player_data(player_id_to_edit, players_db_dict_ref):
    if player_id_to_edit not in players_db_dict_ref:
        print(_("Errore: Giocatore con ID {id} non trovato.").format(id=player_id_to_edit))
    ...
    print(_("\n--- Modifica Giocatore ID: {id} ({first} {last}) ---").format(
        id=original_id, first=player_data_ref.get('first_name'), last=player_data_ref.get('last_name')))
    ...
    if (new_first_name and new_first_name != original_first_name) or \
       (new_last_name and new_last_name != original_last_name):
        if new_first_name and new_last_name:
            ...
            if get_input_with_default(_("Vuoi tentare di rigenerare l'ID in base al nuovo nome/cognome? (s/N)"), "n").lower() == 's':
                ...
                if candidate_new_id and candidate_new_id != original_id and candidate_new_id not in players_db_dict_ref:
                    ...
                    print(_("ID giocatore aggiornato da '{old}' a '{new}'.").format(old=original_id, new=final_id_for_player))
                else:
                    print(_("Errore o collisione nella generazione del nuovo ID. L'ID originale '{id}' verrà mantenuto.").format(id=original_id))
                    players_db_dict_ref[original_id] = temp_player_data_for_id_gen 
        else:
            print(_("Nome e/o cognome non possono essere vuoti. Modifiche a nome/cognome annullate."))
    ...
    try: current_elo_val = float(player_data_ref.get('current_elo', DEFAULT_ELO))
    except ValueError: current_elo_val = DEFAULT_ELO
    player_data_ref['current_elo'] = dgt(_("Elo Corrente (range 0-3500)"), kind="f", fmin=0.0, fmax=3500.0, default=current_elo_val)
    
    player_data_ref['fide_title'] = get_input_with_default(_("Titolo FIDE (es. FM, '' per nessuno)"), player_data_ref.get('fide_title', '')).upper()[:3]
    
    sex_default_edit = player_data_ref.get('sex', 'm')
    while True:
        sex_input_val = get_input_with_default(_("Sesso (m/w)"), sex_default_edit).lower()
        if sex_input_val in ['m', 'w']: player_data_ref['sex'] = sex_input_val; break
        elif not sex_input_val and sex_default_edit: player_data_ref['sex'] = sex_default_edit; break
        elif not sex_input_val and not sex_default_edit: player_data_ref['sex'] = 'm'; break
        print(_("Input non valido."))
            
    fed_default_edit = player_data_ref.get('federation', 'ITA')
    player_data_ref['federation'] = get_input_with_default(_("Federazione (3 lettere)"), fed_default_edit).upper()[:3] or 'ITA'

    fide_id_default_edit = player_data_ref.get('fide_id_num_str', '0')
    new_fide_id_val = get_input_with_default(_("ID FIDE Numerico (cifre, '0' se N/D)"), fide_id_default_edit)
    if not new_fide_id_val.isdigit(): new_fide_id_val = fide_id_default_edit 
    player_data_ref['fide_id_num_str'] = new_fide_id_val or '0'

    birth_date_default_edit_str = player_data_ref.get('birth_date', "") 
    while True:
        bdate_input_val = get_input_with_default(_("Data di nascita ({date_format} o vuoto per cancellare)").format(date_format=DATE_FORMAT_ISO), birth_date_default_edit_str)
    ...
    while True:
        exp_input_str_edit = get_input_with_default(_("Giocatore con esperienza pregressa significativa? (s/n)"), experienced_default_str).lower()
    ...
    for m_key in ['gold', 'silver', 'bronze', 'wood']:
        medals_data_ref[m_key] = dgt(f"{_('Medaglie di')} {m_key.capitalize()}", kind="i", imin=0, imax=999, default=medals_data_ref.get(m_key, 0))
    ...
    while True:
        ...
        if not tournaments_data_ref:
            ...
            for i, t_disp in enumerate(sorted_tournaments_for_display):
                ...
                print(f"     {_('Date')}: {start_disp} - {end_disp} (ID Torneo: {tid_disp})")
        
        print(_("\nOpzioni Storico Tornei:"))
        op_tourn = get_input_with_default(_("  (A)ggiungi, (C)ancella, (F)ine gestione storico: "), "f").strip().lower()

        if op_tourn == 'f': break
        elif op_tourn == 'a':
            print(_("\n  --- Aggiunta nuovo torneo allo storico ---"))
            new_t_entry = {}
            new_t_entry['tournament_name'] = get_input_with_default(_("  Nome torneo: "))
            if not new_t_entry['tournament_name']: continue
            new_t_entry['tournament_id'] = get_input_with_default(_("  ID Torneo (default: generato in automatico): "), 
                                                                        f"{sanitize_filename(new_t_entry['tournament_name'])}_{datetime.now().strftime('%Y%m')}")
            new_t_entry['rank'] = get_input_with_default(_("  Posizione (es. 1, 2, o RIT): "))
            try: new_t_entry['total_players'] = int(get_input_with_default(_("  Numero totale partecipanti: ")))
            except ValueError: new_t_entry['total_players'] = 0
            
            for f_date in ['date_started', 'date_completed']:
                while True:
                    d_str = get_input_with_default(_("  Data {field} ({format}): ").format(field=f_date, format=DATE_FORMAT_ISO))
                    if not d_str: new_t_entry[f_date] = None; break
                    try: datetime.strptime(d_str, DATE_FORMAT_ISO); new_t_entry[f_date] = d_str; break
                    except ValueError: print(_("  Formato data non valido."))
            ...
            print(_("  Torneo aggiunto allo storico."))
             ...
             except (ValueError, IndexError): print(_("  Errore cancellazione."))
...
def main_interactive_db_tool_loop(players_db_main_dict):
    ...
    while True:
        ...
        prompt_msg_main = _("Cerca giocatore (ID, nome/cognome), (L)ista tutti, (A)ggiungi nuovo, (S)alva e esci: ")
        if last_managed_player_id and last_managed_player_id in players_db_main_dict:
             prompt_msg_main = _("Cerca (ID prec: {id}), (L)ista, (A)ggiungi, (S)alva e esci: ").format(id=last_managed_player_id)
        
        search_input_main_val = get_input_with_default(prompt_msg_main, "s").strip().lower()
        if search_input_main_val == 's' or not search_input_main_val: break 
        ...
        if search_input_main_val == 'l':
            if not players_db_main_dict: print(_("Database vuoto."))
            else:
        ...
        if not found_players_list_main:
            print(_("Nessun giocatore trovato per '{search}'.").format(search=search_input_main_val))
            if get_input_with_default(_("Vuoi aggiungere un nuovo giocatore? (S/n) "), "n").lower() == 's':
                if add_new_player(players_db_main_dict): save_players_db(players_db_main_dict)
            continue
            ...
            while True: 
                if current_player_id_main_ops in players_db_main_dict: player_to_manage_item = players_db_main_dict[current_player_id_main_ops]
                else: break

                display_player_details(player_to_manage_item) 
                action_main = get_input_with_default(_("\nAzione: (M)odifica, (C)ancella, (S)eleziona altro/Fine"), "s").lower()

                if action_main == 's': last_managed_player_id = current_player_id_main_ops; break 
                elif action_main == 'c':
                    if get_input_with_default(_("Sicuro di cancellare {first} {last} (ID: {id})? (s/N)").format(
                        first=player_to_manage_item.get('first_name'), last=player_to_manage_item.get('last_name'), id=current_player_id_main_ops), "n").lower() == 's':
            ...
            print(_("\nTrovate {num} corrispondenze. Specifica meglio:").format(num=len(found_players_list_main)))
