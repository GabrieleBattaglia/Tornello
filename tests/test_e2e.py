...
def test_e2e_complex_tournament_flow_bye_withdrawals(tmp_path, monkeypatch):
    ...
    initial_db = {"schema_version": 2, "players": players_db_list}
    with open(db_file, "w", encoding="utf-8") as f:
        json.dump(initial_db, f, indent=4)
    ...
    for round_num in range(1, 8):
        ...
        for idx, match in enumerate(matches):
            if match.get("black_player_id") is None:
                ...
                if bye_player:
                    bye_player.setdefault("results_history", []).append(
                        ResultEntry(
                            round=round_num,
                ...
                _apply_match_result_to_players(
                    torneo_dict, match, res, w_score, b_score
                )

        # Salva il round
        round_obj = Round(
            round=round_num, matches=[Match.from_dict(m) for m in matches]
        )
