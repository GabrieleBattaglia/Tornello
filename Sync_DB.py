...
try:
    ...
    print("Assicurati che la cartella GBUtils sia nel PYTHONPATH o nella stessa directory padre.")
    sys.exit(1)
...
def main():
    ...
    if not os.path.exists(FIDE_DB_LOCAL_FILE):
        print(_("Il database FIDE locale ({}) non è presente.").format(FIDE_DB_LOCAL_FILE))
        if enter_escape(_("Vuoi scaricarlo ora da FIDE.com? (Richiede alcuni minuti) (INVIO|ESCAPE)")):
            if not aggiorna_db_fide_locale():
                print(_("Scaricamento annullato o fallito. Sincronizzazione impossibile."))
