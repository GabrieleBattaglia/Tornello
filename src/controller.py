...
from config import FIDE_DB_LOCAL_FILE, PLAYER_DB_FILE, DATE_FORMAT_ISO, DEFAULT_K_FACTOR, DEFAULT_ELO
from utils import format_date_locale, sanitize_filename, create_backup, parse_flexible_date
from db_players import load_players_db, sincronizza_db_personale, aggiorna_db_fide_locale
...
class TournamentController:
    ...
    def _check_fide_db(self) -> None:
        ...
        if not db_fide_esiste:
            ...
            if self.ui.confirm(_("Vuoi scaricarlo ora? (L'operazione potrebbe richiedere alcuni minuti)")):
                if aggiorna_db_fide_locale():
                    db_fide_appena_aggiornato = True
                    self.ui.show_message(_("Database FIDE locale aggiornato con successo."))
        else:
            try:
                ...
                self.ui.show_message(_("Info: Il tuo database FIDE locale ha {} giorni.").format(file_age_days))
                if file_age_days >= 32:
                    self.ui.show_message(_("Essendo trascorsi più di 32 giorni dall'ultimo download, potrebbe essere stato rilasciato un aggiornamento."))
                    if self.ui.confirm(_("Si consiglia di aggiornarlo. Vuoi scaricare la versione più recente?")):
                        if aggiorna_db_fide_locale():
                            db_fide_appena_aggiornato = True
            except Exception as e:
                self.ui.show_error(_("Errore nel controllare la data del file DB FIDE locale: {}").format(e))
        ...
        if not potential_tournament_files:
            ...
            try:
                ...
                self.ui.show_message(
                    _("Info: impossibile leggere i dettagli da '{filename}' ({error}). Uso il nome del file.").format(
                        filename=os.path.basename(single_found_filepath), error=e
                    )
                )
            ...
            choice_str = self.ui.input_text(
                _("Vuoi caricare '{name}'? (Premi INVIO per sì, o inserisci il nome di un NUOVO torneo da creare): ").format(
                    name=single_tournament_name_guess
                )
            ).strip()
            ...
            if not choice_str:
                ...
                self.ui.show_message(_("Caricamento di '{name}'...").format(name=single_tournament_name_guess))
                torneo_dict = load_tournament(self.active_filename)
                if torneo_dict:
                    self.tournament = Tournament.from_dict(torneo_dict)
                else:
                    self.ui.show_error(_("Errore fatale nel caricamento del torneo '{filename}'.").format(filename=self.active_filename))
                    if self.ui.confirm(_("Vuoi creare un nuovo torneo?")):
                        deve_creare_nuovo_torneo = True
                    else:
                        self._exit_program(0)
            ...
            for idx, filepath in enumerate(potential_tournament_files):
                try:
                    ...
                    self.ui.show_message(
                        _(" {num}. {name} (dal {start_date} al {end_date}) - File: {filename}").format(
                            num=idx + 1, name=t_name, start_date=start_display, end_date=end_display, filename=os.path.basename(filepath)
                        )
                    )
                except Exception as e:
                    self.ui.show_error(_(" Errore durante la lettura dei metadati da {filename}: {error} (file saltato)").format(
                        filename=os.path.basename(filepath), error=e
                    ))

            if not tournament_options:
                self.ui.show_message(_("Nessun torneo valido trovato nonostante la presenza di file. Si procederà con la creazione."))
                deve_creare_nuovo_torneo = True
            else:
                ...
                if suspended_tournaments:
                    ...
                    for st in suspended_tournaments:
                        self.ui.show_message(_(" - '{name}' (File: {filename})").format(name=st["name"], filename=os.path.basename(st["filepath"])))

                    if len(suspended_tournaments) == 1:
                        if self.ui.confirm(_("Vuoi riprendere la creazione di '{name}'?").format(name=suspended_tournaments[0]["name"])):
                            self.active_filename = suspended_tournaments[0]["filepath"]
                            torneo_dict = load_tournament(self.active_filename)
                            self.tournament = Tournament.from_dict(torneo_dict)
                            deve_creare_nuovo_torneo = False
                            self.ui.show_message(_("Ripresa creazione torneo '{name}'...").format(name=self.tournament.name))
                    else:
                        self.ui.show_message(_("Ci sono più tornei in stato sospeso. Selezionali dal menu per riprenderli."))

                if not self.tournament:
                    options_list = [opt["name"] for opt in tournament_options] + [_("Crea un nuovo torneo")]
                    idx_choice = self.ui.select_option(_("Scegli un torneo da caricare o creane uno nuovo"), options_list)
                    if idx_choice < len(tournament_options):
                        chosen = tournament_options[idx_choice]
                        self.active_filename = chosen["filepath"]
                        self.ui.show_message(_("Caricamento di '{name}'...").format(name=chosen["name"]))
                        torneo_dict = load_tournament(self.active_filename)
                        if torneo_dict:
                            self.tournament = Tournament.from_dict(torneo_dict)
                        else:
                            self.ui.show_error(_("Errore nel caricamento del torneo. Riprova."))
                            self._exit_program(1)
                    else:
                        deve_creare_nuovo_torneo = True

        # Gestione ripresa torneo sospeso
        if self.tournament and self.tournament.schema_version == 1 and getattr(self.tournament, "creation_suspended", False):
    ...
    def _resume_suspended_creation(self) -> None:
        self.ui.show_message(_("\nRipresa inserimento giocatori per '{name}'...").format(name=self.tournament.name))
        res = self.ui.input_players(self.players_db, self.tournament.players, self.tournament, self.active_filename)
        if res is None:
            self._exit_program(0)
        ...
        if not self.ui.confirm_player_list(self.tournament, self.players_db):
            self.ui.show_message(_("Creazione torneo annullata a causa di problemi con la lista giocatori."))
            self._exit_program(0)
    ...
    def _create_new_tournament(self, suggested_name: Optional[str] = None) -> None:
        ...
        if suggested_name:
            self.ui.show_message(_("Nome suggerito per il nuovo torneo: '{name}'").format(name=suggested_name))
            sanitized = sanitize_filename(suggested_name)
            prospective = f"Tornello - {sanitized}.json"
            if os.path.exists(prospective):
                if self.ui.confirm(_("Un file torneo '{filename}' con questo nome esiste già. Sovrascriverlo?").format(filename=prospective)):
        ...
        while not new_name:
            input_name = self.ui.input_text(_("Inserisci il nome del nuovo torneo")).strip()
            if not input_name:
            ...
            prospective = f"Tornello - {sanitized}.json"
            if os.path.exists(prospective):
                if not self.ui.confirm(_("Un file torneo '{filename}' con questo nome esiste già. Sovrascriverlo?").format(filename=prospective)):
                    continue
            new_name = input_name
            self.active_filename = prospective
        ...
        while not start_date:
            ...
            inp_date = self.ui.input_text(
                _("Inserisci data inizio ({date_format}) [Default: {default_date}]").format(
                    date_format=DATE_FORMAT_ISO, default_date=oggi_str_locale
                )
            ).strip()
            if not inp_date:
            ...
            try:
                ...
                self.ui.show_error(_("Formato data non valido. Usa {date_format} o AAAAMMGG.").format(date_format=DATE_FORMAT_ISO))
        ...
        while not end_date:
            ...
            inp_date = self.ui.input_text(
                _("Inserisci data fine ({date_format}) [Default: {default_date}]").format(
                    date_format=DATE_FORMAT_ISO, default_date=default_end_locale
                )
            ).strip()
            if not inp_date:
            ...
            try:
                ...
                if end_dt < start_dt:
                    self.ui.show_error(_("La data di fine non può essere precedente alla data di inizio."))
                    continue
                end_date = inp_date
            except ValueError:
                self.ui.show_error(_("Formato data non valido. Usa {date_format}.").format(date_format=DATE_FORMAT_ISO))
            except OverflowError:
                self.ui.show_error(_("La data calcolata risulta troppo lontana nel futuro."))

        # Turni
        total_rounds = self.ui.input_int(_("Inserisci il numero totale dei turni"), min_val=1)

        # Dettagli aggiuntivi
        site = self.ui.input_text(_("Luogo del torneo [Default: Luogo Sconosciuto]")).strip() or _("Luogo Sconosciuto")
        fed_code = self.ui.input_text(_("Federazione organizzante (codice 3 lettere) [Default: ITA]")).strip().upper() or "ITA"
        fed_code = fed_code[:3]
        ...
        if round_dates_raw is None:
            self.ui.show_error(_("Errore fatale nel calcolo delle date dei turni. Creazione torneo annullata."))
            self._exit_program(1)

        round_dates = [RoundDate.from_dict(rd) for rd in round_dates_raw]
        tournament_id = f"{sanitize_filename(new_name)}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ...
        existing_players = []
        while True:
            res = self.ui.input_players(self.players_db, existing_players, self.tournament, self.active_filename)
            if res is None:
                # Sospeso
                self._exit_program(0)
            self.tournament.players = res
        ...
        self.ui.show_message(_("Il valore suggerito è: {val}").format(val=valore_bye_suggerito))
        self.ui.show_message("-" * 30)
        if self.ui.confirm(_("Accetti il valore suggerito? (No = usa {alt_val})").format(alt_val=valore_alternativo)):
            self.tournament.bye_value = valore_bye_suggerito
        else:
            self.tournament.bye_value = valore_alternativo

        self.ui.show_message(_("Valore del BYE impostato a: {val}").format(val=self.tournament.bye_value))

        min_req = self.tournament.total_rounds + 1
        if len(self.tournament.players) < min_req:
            self.ui.show_error(
                _("Numero insufficiente di giocatori ({num_players}) per {num_rounds} turni. Torneo annullato.").format(
                    num_players=len(self.tournament.players), num_rounds=self.tournament.total_rounds
                )
            )
            self._exit_program(0)
        ...
        if matches_r1_raw is None:
            self.ui.show_error(_("ERRORE CRITICO: Fallimento generazione abbinamenti Turno 1. Torneo non avviato."))
            self._exit_program(1)

        matches_r1 = [Match.from_dict(m) for m in matches_r1_raw]
        self.ui.play_sound("nuovo_turno", self.tournament)
        self.ui.show_message(_("Registrazione risultati automatici per il Turno 1 (BYE)..."))
        ...
        for m in matches_r1:
            if m.result == "BYE":
                ...
                if bye_player:
                    ...
                    bye_player.results_history.append(ResultEntry(
                        round=1, opponent_id="BYE_PLAYER_ID", color=None, result="BYE", score=self.tournament.bye_value
                    ))
                    # Aggiorna contatori
                    bye_player.received_bye_count += 1
                    bye_player.received_bye_in_round.append(1)
                    self.ui.show_message(
                        _(" > Giocatore {name} (ID: {id}) ha ricevuto un BYE. Punti e storico aggiornati.").format(
                            name=bye_player.first_name, id=bye_player.id
                        )
                    )
        ...
        if not self.ui.confirm(_("Vuoi che i dati vengano salvati in {}?").format(default_path)):
            while True:
                custom_path = self.ui.input_text(_("Inserisci il percorso di salvataggio desiderato")).strip()
                if not custom_path:
                ...
                try:
                    ...
                    self.active_filename = os.path.join(custom_path, f"Tornello - {sanitized_name}.json")
                    # Rimuoviamo il vecchio se presente
                    break
                except Exception as e:
                    self.ui.show_error(_("Percorso non valido o non scrivibile: {}").format(e))
    ...
    def _main_loop(self) -> None:
        ...
        self.ui.show_message(_("Copyright 2026, dedicato all'ASCId e al gruppo Scacchierando."))
        ...
        try:
            while True:
                ...
                if curr_round > tot_rounds:
                    self.ui.show_message(_("\nIl torneo '{name}' si è concluso.").format(name=self.tournament.name))
                    break

                self.ui.show_message(_("\n--- Gestione Turno {round_num} ---").format(round_num=curr_round))
                self.ui.display_tournament_status(self.tournament)

                round_data = next((r for r in self.tournament.rounds if r.round == curr_round), None)
                if not round_data:
                    self.ui.show_error(_("ERRORE CRITICO: Dati turno {round_num} non trovati!").format(round_num=curr_round))
                    break

                round_completed = all(m.result is not None or m.black_player_id is None for m in round_data.matches) if round_data.matches else False
                ...
                if not round_completed:
                    ...
                    if not self.ui.confirm(_("Vuoi continuare?")):
                        self.ui.show_message(_("Salvataggio finale prima dell'uscita..."))
                        self._save_state()
                        break
                else:
                    self.ui.show_message(_("\nTurno {round_num} completato.").format(round_num=curr_round))
                    ...
                    if curr_round == tot_rounds:
                        ...
                        if self._finalize_tournament():
                            ...
                            self.ui.show_error(_("\n--- ERRORE durante la Finalizzazione del Torneo ---"))
                            self._save_state()
                        break
                    else:
                        next_round = curr_round + 1
                        if self.ui.confirm(_("\nVuoi procedere e generare gli abbinamenti per il Turno {round_num}?").format(round_num=next_round)):
                            self.tournament.current_round = next_round
                            self.ui.show_message(_("Generazione abbinamenti per il Turno {round_num}...").format(round_num=next_round))
                            ...
                            if next_matches_raw is None:
                                user_action = handle_bbpairings_failure(
                                    torneo_dict, next_round, "Errore durante la generazione."
                                )
                                if user_action == "time_machine":
                                    ...
                                    if time_machine_torneo(torneo_dict):
                                        ...
                                        self.ui.show_message(_("Time Machine annullata o fallita. Uscita per sicurezza."))
                                        break
                                    continue
                            ...
                            next_matches = [Match.from_dict(m) for m in next_matches_raw]
                            self.ui.play_sound("nuovo_turno", self.tournament)
                            self.ui.show_message(_("Registrazione risultati automatici per il Turno {round_num} (BYE)...").format(round_num=next_round))
                            ...
                            for m in next_matches:
                                if m.result == "BYE":
                                    ...
                                    if bye_player:
                                        bye_player.points += self.tournament.bye_value
                                        bye_player.results_history.append(ResultEntry(
                                            round=next_round, opponent_id="BYE_PLAYER_ID", color=None, result="BYE", score=self.tournament.bye_value
                                        ))
                                        bye_player.received_bye_count += 1
                                        bye_player.received_bye_in_round.append(next_round)
                                        self.ui.show_message(
                                            _(" > Giocatore {name} (ID: {id}) ha ricevuto un BYE. Punti e storico aggiornati.").format(
                                                name=bye_player.first_name, id=bye_player.id
                                            )
                            ...
                            self.tournament.rounds.append(Round(round=next_round, matches=next_matches))
                            self._save_state()
                            self.ui.show_message(_("Turno {round_num} generato e salvato.").format(round_num=next_round))
                        else:
                            self.ui.show_message(_("Generazione prossimo turno annullata. Salvataggio stato attuale."))
                            self._save_state()
                            break
            ...
            self.ui.show_error(_("\nERRORE CRITICO NON GESTITO nel flusso principale: {error}").format(error=e))
            import traceback
            traceback.print_exc()
            if self.tournament:
                self._save_state()
    ...
    def _finalize_tournament(self) -> bool:
        ...
        # Creazione backup
        self.ui.show_message(_("Creazione backup di sicurezza prima dell'archiviazione..."))
        ...
        if self.active_filename and os.path.exists(self.active_filename):
            backup_torneo_ok = create_backup(self.active_filename, "pre_finalize_torneo")

        if not backup_db_ok or not backup_torneo_ok:
            self.ui.show_message(_("ATTENZIONE: Fallita la creazione di uno o più file di backup. Procedo ugualmente..."))

        self.tournament.update_players_dict()
        if not self.tournament.players:
            self.ui.show_error(_("Nessun giocatore nel torneo, impossibile finalizzare."))
            return False
        ...
        for p in self.tournament.players:
            ...
            for r in p.results_history:
                if r.opponent_id and r.opponent_id != "BYE_PLAYER_ID" and r.score is not None:
                    games_count += 1
            p.games_this_tournament = games_count

        # Fase 2: Calcola spareggi
        self.ui.show_message(_("Ricalcolo finale Buchholz, ARO, Performance Rating, Variazione Elo..."))
        # Riconvertiamo a dizionario temporaneamente per le vecchie librerie pure di calcolo
        torneo_dict = self.tournament.to_dict()
        torneo_dict["players_dict"] = {p.id: p.to_dict() for p in self.tournament.players}
        ...
        return finalize_tournament(self.tournament.to_dict(), self.players_db, self.active_filename)
