...
class CLIAdapter(UIAdapter):
        ...
        while True:
            ...
            try:
                ...
                if min_val is not None and val < min_val:
                    self.show_error(
                        _("Il valore deve essere almeno {min_val}.").format(
                            min_val=min_val
                        )
                    )
                    continue
                if max_val is not None and val > max_val:
                    self.show_error(
                        _("Il valore non può superare {max_val}.").format(
                            max_val=max_val
                        )
                    )
                    continue
    ...
    def update_match_results(self, tournament: Tournament) -> bool:
        ...
        if res:
            ...
            tournament.players = [Player.from_dict(p) for p in torneo_dict["players"]]
            tournament.rounds = [
                Round.from_dict(r) for r in torneo_dict.get("rounds", [])
            ]
            tournament.current_round = torneo_dict.get(
                "current_round", tournament.current_round
            )
            tournament.next_match_id = torneo_dict.get(
