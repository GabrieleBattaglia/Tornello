...
class BackupCleanupDialog(wx.Dialog):
    ...
    def __init__(self, parent, settings):
        ...
        super().__init__(parent, title=title, size=(800, 550), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
    ...
    def _init_ui(self):
        ...
        self.stats_text = wx.TextCtrl(panel, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_RICH2, size=(-1, 100))
        ...
        self.lbl_list = wx.StaticText(panel, label=_("Elenco file di backup (dal più vecchio al più recente):"))
        vbox.Add(self.lbl_list, 0, wx.LEFT | wx.RIGHT | wx.BOTTOM, 5)
        
        # 3. Lista dei file di backup
        self.list_ctrl = wx.ListCtrl(panel, style=wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.LC_HRULES | wx.LC_VRULES)
        ...
        self.btn_delete_old = wx.Button(panel, label=_("Elimina consigliati (>18 mesi)"))
    ...
    def populate_list(self):
        ...
        if total_files > 0:
            self.list_ctrl.SetItemState(0, wx.LIST_STATE_SELECTED | wx.LIST_STATE_FOCUSED, wx.LIST_STATE_SELECTED | wx.LIST_STATE_FOCUSED)

    def on_delete_selected(self, event):
        """Elimina il file attualmente selezionato nella lista."""
        selected_idx = self.list_ctrl.GetNextItem(-1, wx.LIST_NEXT_ALL, wx.LIST_STATE_SELECTED)
        if selected_idx == -1:
            msg = _("Nessun file selezionato. Seleziona un file dall'elenco per poterlo eliminare.")
        ...
        msg = _("Sei sicuro di voler spostare nel cestino il file di backup '{name}'?").format(name=file_info["name"])
        ...
        if dlg.ShowModal() == wx.ID_YES:
            ...
            if delete_file_to_trash(file_info["path"]):
                ...
                err_msg = _("Impossibile eliminare il file '{name}'.").format(name=file_info["name"])
    ...
    def on_delete_old(self, event):
        ...
        msg = _("Sei sicuro di voler spostare nel cestino tutti i {count} file di backup più vecchi di 18 mesi?").format(count=len(self.old_files_info))
        dlg = AccessibleMsgDialog(self, _("Conferma Eliminazione Consigliata"), msg, style=wx.YES_NO)
        if dlg.ShowModal() == wx.ID_YES:
            dlg.Destroy()
            failed = []
    ...
    def on_empty_folder(self, event):
        ...
        msg = _("ATTENZIONE: Sei sicuro di voler spostare nel cestino TUTTI i file di backup contenuti nella cartella?")
        dlg = AccessibleMsgDialog(self, _("Conferma Svuotamento Cartella"), msg, style=wx.YES_NO)
        if dlg.ShowModal() == wx.ID_YES:
            dlg.Destroy()
            failed = []
