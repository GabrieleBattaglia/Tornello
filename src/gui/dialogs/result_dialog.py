...
def format_date_italian(d):
    ...
    day_name = days_it[d.weekday()]
    month_name = months_it[d.month - 1]
...
class ScheduleDialog(wx.Dialog):
    ...
    def __init__(self, parent, schedule_info, settings, tournament_data):
        super().__init__(parent, title=_("Pianificazione Partita"), size=(500, 600), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
    ...
    def _init_ui(self):
        ...
        if current_round_period_info and current_round_period_info.get("end_date"):
            try:
                end_date = datetime.datetime.strptime(current_round_period_info.get("end_date"), "%Y-%m-%d").date()
            except ValueError:
                pass
                
        if not end_date and self.tournament_data.get("end_date"):
            try:
                end_date = datetime.datetime.strptime(self.tournament_data.get("end_date"), "%Y-%m-%d").date()
            except ValueError:
                pass
        ...
        self.choice_min = wx.Choice(panel, choices=[f"{m:02d}" for m in range(0, 60, 5)])
...
class ResultDialog(wx.Dialog):
    ...
    def __init__(self, parent, white_name, black_name, white_id, black_id, board_num, current_result, schedule_info, settings, pgn_text="", disable_result_change=False):
        ...
        super().__init__(parent, title=title, size=(550, 650), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
    ...
    def _init_ui(self):
        ...
        if self.schedule_info.get("date") and self.schedule_info.get("time"):
            ...
            if self.schedule_info.get("channel"):
                sched_lbl += f" | Sala/URL: {self.schedule_info['channel']}"
            if self.schedule_info.get("arbiter"):
                sched_lbl += f" | Arbitro: {self.schedule_info['arbiter']}"
        ...
        self.options = [
            ("1-0", f"1 - 0 (Vince {self.white_name})"),
            ("0-1", f"0 - 1 (Vince {self.black_name})"),
            ("1/2-1/2", "1/2 - 1/2 (Patta)"),
            ("1-F", f"1 - F (Forfait {self.white_name} / Assenza {self.black_name})"),
            ("F-1", f"F - 1 (Forfait {self.black_name} / Assenza {self.white_name})"),
            ("0-0F", "0 - 0F (Assenza Doppia)")
        ]
