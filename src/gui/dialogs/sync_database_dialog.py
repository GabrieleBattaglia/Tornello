...
class SyncDatabaseDialog(wx.Dialog):
    ...
    def __init__(self, parent, settings):
        ...
        super().__init__(parent, title=title, size=(750, 500), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
        ...
        for player_id, local_player in self.players_db.items():
            ...
            if fide_id_str and fide_id_str != "0":
                ...
                if p_first and p_last:
                    matches = [
                        fp for fp in self.fide_db.values()
    ...
    def _init_ui(self):
        ...
        summary_text = ""
        if not self.changes:
            summary_text = _("Il tuo database personale è già perfettamente sincronizzato con il DB FIDE locale!")
        else:
            summary_text = (
                _("Trovate modifiche disponibili per {num} giocatori:\n").format(num=len(self.changes)) +
                f" - {self.stats['id_associations']} " + _("nuovi ID FIDE da associare\n") +
                f" - {self.stats['elo_updates']} " + _("aggiornamenti di punteggio Elo\n") +
                f" - {self.stats['other_updates']} " + _("aggiornamenti di dati anagrafici/titoli\n\n") +
                _("Scegli se aggiornare tutto in massa (gli omonimi multipli andranno risolti singolarmente)\n"
                  "oppure valutare ogni singolo giocatore passo-passo.")
            )
            
        self.txt_summary = wx.TextCtrl(panel, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_RICH2, size=(-1, 200))
    ...
    def on_bulk_sync(self, event):
        ...
        for idx, change in enumerate(self.changes):
            dlg_prog.Update(idx, _("Aggiornamento {name}...").format(
        ...
        if ambiguous_list:
            msg = _("Aggiornati {num} giocatori. Ci sono {amb} omonimi con ID ambiguo da risolvere.").format(
                num=applied_count, amb=len(ambiguous_list)
            )
            wx.MessageBox(msg, _("Risoluzione Omonimi"), wx.ICON_INFORMATION)
            self._resolve_ambiguous_changes(ambiguous_list)
        else:
            msg = _("Sincronizzazione massiva completata con successo! Aggiornati {num} giocatori.").format(num=applied_count)
    ...
    def _resolve_ambiguous_changes(self, ambiguous_list):
        ...
        for change in ambiguous_list:
            ...
            dlg = wx.SingleChoiceDialog(
                self,
                _("Seleziona il record FIDE corretto per {name}:").format(name=f"{player.get('last_name')} {player.get('first_name')}"),
        ...
        save_players_db(self.players_db)
        wx.MessageBox(_("Tutti gli omonimi sono stati risolti e salvati."), _("Sincronizzazione Completata"), wx.ICON_INFORMATION)
        self.EndModal(wx.ID_OK)
